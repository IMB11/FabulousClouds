...
public class SkyCoverGenerators {
    ...
    public static void clearSkyUpdate(Random random, SimplexNoise noiseSampler, NativeImage image, List<CloudTexture.PixelCoordinate> pixels, double cloudiness) {
    ...
    public static void normalSkyGenerator(SimplexNoise noiseSampler, NativeImage image, double cloudiness) {
        for (int x = 0; x < 256; x++) {
            for (int z = 0; z < 256; z++) {
                if (noiseSampler.getValue(x / 16.0, 0, z / 16.0) * 2.5 < cloudiness || image.getPixelRGBA(x, z) != 0) {
    ...
    public static void normalSkyUpdate(Random random, SimplexNoise noiseSampler, NativeImage image, List<CloudTexture.PixelCoordinate> pixels, double cloudiness) {
